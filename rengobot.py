...
admins = [610932957276012544]  # Ohasa
...
async def play(ctx, arg):
    ...
    if (state[i][1] == "queue" and user.id != state[i][4][colour][0]) or (state[i][1] == "teachers" and (
            (colour == 0 and user.id != state[i][4][0][0]) or (colour == 1 and user.id not in state[i][4][1]))):
...
async def edit(ctx, arg):  # literally play but with fewer things
    ...
    if len(state[i][2]) == 0 or state[i][2][-1] != user.id or datetime.now() - datetime.strptime(state[i][3][-1],
                                                                                                 format) > timedelta(
            minutes=5):
...
async def board(ctx):
    ...
    os.system("sgf-render --style fancy --label-sides nesw -o " + str(channel_id) + ".png -n last " + str(
        channel_id) + ".sgf")
...
async def queue(ctx):
    ...
    if len(state[i][4][colour]) > len(state[i][4][1 - colour]):
        ...
        last_player = state[i][4][1 - colour][-1]
